...
class View
{
    ...
    public function __construct(string $layout, string $template, array $meta)
    {
        ...
        $this->meta = $meta;
        // Если передан шаблон, мы возьмем его. В противном случае если передана строка тогда мы возьмём по умолчанию шаблон
    ...
    public function show(array $data): void
    {
        ...
        // Создаем динамический путь к виду
...
//            echo $content;
        ...
        if($this->layout) {
            ...
            // Так как у нас динамически создается путь и название файла мы можем менять шаблон по усмотрению
            // Указав в контролере переназначив переменно layout по примеру  $this->layout = 'test';
            $layoutFile = VIEW . "/layouts/{$this->layout}.template.php";
            // проверяем что получилось
//            var_dump($layoutFile);

            // Проверка если файл. Если есть, то подключаем, Если нет, то выкидываем ошибку
            ...
            throw new \Exception("Не пришло название шаблона ", 500);
        }
    }
